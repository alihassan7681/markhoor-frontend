...
const StudentRecords = () => {
  ...
  useEffect(() => {
    api.get('/teacher/reports/students')
    ...
    <div className="space-y-8 animate-fade-in pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tight">
            Student <span className="text-[#7c3aed]">Records</span>
          </h1>
          <p className="text-slate-500 font-medium tracking-tight">Complete history of student test performances.</p>
        </div>

        <div className="relative group">
          <FaSearch className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-[#7c3aed] transition-colors" />
          <input 
            type="text" 
            placeholder="Search by name or serial..." 
            className="pl-11 pr-6 py-3 rounded-2xl bg-white border border-slate-100 shadow-sm focus:ring-4 focus:ring-[#7c3aed]/10 focus:border-[#7c3aed] outline-none transition-all w-full md:w-80 font-bold text-sm text-slate-700"
            value={search}
      ...
      <div className="grid gap-6">
        {filtered.map((student) => (
          <div key={student.id} className="glass-card rounded-[32px] p-8 transition-all hover:shadow-xl hover:shadow-[#7c3aed]/5 group overflow-hidden relative">
            {/* Decoration */}
            <div className="absolute top-0 right-0 w-32 h-32 bg-[#7c3aed]/5 rounded-full blur-3xl -translate-x-1/2 -translate-y-1/2 group-hover:bg-[#7c3aed]/10 transition-colors"></div>
            
            <div className="flex flex-col lg:flex-row lg:items-center gap-8 relative z-10">
              {/* Student Basic Info */}
              <div className="lg:w-1/4 space-y-4">
                <div className="flex items-center gap-4">
                  <div className="h-14 w-14 rounded-2xl bg-slate-100 flex items-center justify-center text-[#7c3aed] text-2xl group-hover:bg-[#7c3aed] group-hover:text-white transition-all duration-500">
                    <FaGraduationCap />
                  </div>
                  <div>
              ...
              <div className="lg:flex-1">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                  ...
                  <div className="p-4 rounded-2xl bg-[#7c3aed]/5 border border-[#7c3aed]/10">
                    <p className="text-[10px] font-black uppercase tracking-widest text-[#7c3aed]/60 mb-1">Last Score</p>
                ...
                <div className="space-y-2">
                  <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 px-1 mb-2 flex items-center gap-2">
                    <FaFileInvoice className="text-[#7c3aed]" /> Detailed History
                  </p>
